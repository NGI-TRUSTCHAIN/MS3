...
export function testEVMWalletInterface(wallet: IEVMWallet, skipConnectivity: boolean = false) {
  ...
  if (!wallet) {
    logger.warning('EVM Wallet instance is undefined, skipping EVM-specific tests');
    return;
  }
  ...
  describe('IEVMWallet Extension Tests', () => {
    ...
    if (!skipConnectivity) {
      describe('Functional Tests (EVM Specific)', () => {
        ...
        logger.warning('No accounts available for EVM functional tests. Trying requestAccounts...');
              accounts = await wallet.getAccounts();
            }
            if (accounts.length === 0) {
        logger.crit('!!! CRITICAL: No accounts found for EVM functional tests. !!!');
            }
          } catch (error) {
      logger.error('Error getting accounts before EVM tests:', error);
          }
        });
      ...
      logger.warning('Failed to get gas price:', error);
          }
        });

        it('should estimate gas for a transaction using GenericTransactionData and return bigint', async () => {
          if (accounts.length === 0) {
      logger.warning('Skipping token balance test - no accounts available.');
            return;
          }
          try {
            ...
            logger.warning('Failed to estimate gas:', error);
          }
        });
        ...
        it('should attempt to get a transaction receipt for a dummy hash and return null', async () => {
          ...
          try {
            ...
            logger.info(`getTransactionReceipt returned null for dummy hash ${dummyTxHash}, as expected.`);
          } catch (error) {
            ...
            logger.warning('getTransactionReceipt threw an error for dummy hash (might be provider specific):', error);
      ...
      logger.warning('Skipping token balance test - no accounts available.');
          ...
          if (testTokenAddress) {
      logger.notice(`Attempting to get balance for configured TEST_TOKEN_ADDRESS: ${testTokenAddress}`);
            try {
              const balance = await wallet.callContract({
        ...
        logger.notice(`Token balance for ${testTokenAddress}: ${balance}`);
            } catch (error) {
        logger.warning(`Failed to get token balance for ${testTokenAddress}:`, error);
            }
          } else {
      logger.warning('TEST_TOKEN_ADDRESS not set in env. Testing with a deliberately invalid token address.');
            const invalidTokenAddress = `0x${'1'.repeat(40)}`; // An address that's unlikely to be a token
            try {

              const balance = await wallet.callContract({
                contractAddress: invalidTokenAddress,
                args: [accounts[0]],
                abi: ["function balanceOf(address owner) view returns (uint256)"],
                method: 'balanceOf'
              });
        ...
        logger.notice(`Token balance for invalid address ${invalidTokenAddress} is '0', as expected.`);
            } catch (error: any) {
        ...
        logger.warning(`Failed to get token balance for invalid address ${invalidTokenAddress} (this might be expected): ${error.message}`);
        ...
        it('should support EIP-712 typed data signing using EIP712TypedData structure', async () => {
          if (accounts.length === 0) {
            logger.warning('Skipping signTypedData test - no accounts available.');
            return;
          }
          try {
            ...
            logger.warning('Failed to sign typed data:', error);
          }
        });
      });
    }
