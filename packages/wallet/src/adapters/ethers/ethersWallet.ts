...
// import { AdapterArguments, AdapterError, NetworkConfig, NetworkHelper, PrivateKeyHelper, WalletErrorCode } from "@m3s/common";
// import { EIP712Validator } from "../../helpers/signatures.js";
// import { toWei, toBigInt } from "../../helpers/units.js";
...
//     const keyHelper = new PrivateKeyHelper()
//     // ✅ Process and validate private key in create() method
//     let processedPrivateKey: string;
//     const isValid = keyHelper.isValidEvmPrivateKey(args.options?.privateKey as any)
...
//       processedPrivateKey = keyHelper.generatePrivateKey()
//       // const randomWallet = ethers.Wallet.createRandom();
//       // processedPrivateKey = randomWallet.privateKey;
//       console.debug(`[EvmWalletAdapter.create] Generated new random wallet internally.`);
//     }
...
//     if (!this.wallet) throw new Error('Wallet not created');

//     console.debug("[EvmWalletAdapter Initialize] Setting up provider connections...");

//     // if the user did _not_ pass a provider at all, we just mark initialized
//     const opts = this.config.options?.provider;
//     if (!opts) {
//       console.debug("[EvmWalletAdapter Initialize] No config.provider provided. Provider not set.");
...
//     try {
//       this.provider = await NetworkHelper
//         .getInstance()
//         .getProvider(opts, opts.rpcUrls ?? [], opts.chainId);

//       console.debug(
//         `[EvmWalletAdapter Initialize] Set wallet.provider to ${this.provider ? 'attached' : 'null as no initial provider'
//         }.`
//       );
...
//       console.log("EvmWalletAdapter initialized successfully.");
//     } catch (err: any) {
//       // swallow any provider‐resolution errors at init: we'll handle them in setProvider(...)
//       console.debug("[EvmWalletAdapter Initialize] No provider available. _connected will be false.");
...
//         formattedAmount: ethers.formatUnits(balanceWei, networkConfig.decimals)
//       };
//     } catch (error) {
...
//     if (!this.provider) {
//       return toBigInt(3000000); // Default fallback
//     }

//     try {
//       // Simple checks first
//       if (!txRequest.data || txRequest.data === "0x") {
//         return toBigInt(21000); // ETH transfer
//       }
...
//       if (!txRequest.to) return toBigInt(5000000);  // Contract deployment
//       return toBigInt(3000000); // Contract call
//     }
//   }
...
//       chainIdBigInt = toBigInt(tx.options.chainId);
...
//     console.log('ESTIMATE GAS TX', tx)
...
//       console.log('ESTIMATE GAS TX CONVERTED', txForEstimate)
