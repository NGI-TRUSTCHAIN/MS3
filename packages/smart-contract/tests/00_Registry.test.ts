...
import { Capability, Ms3Modules, getRequirements, getEnvironments, RuntimeEnvironment } from '@m3s/shared';
...
describe('Smart Contract Auto-Generation System Tests', () => {
  ...
  describe('Static Cross-Package Compatibility Matrix', () => {
    // ✅ ADD THIS HOOK: Merge registries before running these tests.
    beforeAll(async () => {
      // Dynamically import other packages to trigger their registration
      const { walletRegistry } = await import('@m3s/wallet');
      const { crossChainRegistry } = await import('@m3s/crosschain');

      // Merge them into the smart-contract registry for the tests
      registry.mergeRegistry(walletRegistry);
      registry.mergeRegistry(crossChainRegistry);
    });
