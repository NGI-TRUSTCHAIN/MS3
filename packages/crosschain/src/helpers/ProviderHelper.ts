...
import { AdapterError, NetworkHelper, WalletErrorCode } from '@m3s/shared';
...
function createM3sViemAccount(wallet: IEVMWallet, address: Address): LocalAccount<'custom', Address> {
    return {
        ...
        async signMessage({ message }: { message: SignableMessage }): Promise<Hex> {
            const messageContent = typeof message === 'string' ? message : (message.raw as string);
        ...
        >(
            ...
            if (!wallet || typeof wallet.signTypedData !== 'function') {
                ...
                throw new AdapterError('M3S wallet is not configured for signTypedData', { methodName: 'signTypedData', code: WalletErrorCode.InvalidInput });
            }
            ...
            if (!parameters.domain) {
                ...
                throw new AdapterError('Domain is missing in typed data for signing.', { methodName: 'signTypedData', code: WalletErrorCode.InvalidInput });
            }
            if (!parameters.primaryType) {
                ...
                console.error(errMsg);
                throw new AdapterError('PrimaryType is missing in typed data for signing.', { methodName: 'signTypedData', code: WalletErrorCode.InvalidInput });
            }
            ...
            const m3sPayload: EIP712TypedData = {
                ...
                types: otherTypes as unknown as EIP712TypedData['types'],
...
export class M3SLiFiViemProvider {
    ...
    static async create(
        ...
        if (!m3sWallet.isInitialized()) {
            throw new AdapterError("M3S Wallet must be initialized before creating M3SLiFiViemProvider.", { methodName: 'create', code: WalletErrorCode.WalletNotConnected });
        }
        if (!initialNetworkConfig || !initialNetworkConfig.rpcUrls || initialNetworkConfig.rpcUrls.length === 0) {
            throw new AdapterError("Valid initialNetworkConfig with rpcUrls is required for M3SLiFiViemProvider.", { methodName: 'create', code: WalletErrorCode.InvalidInput });
        }

        const accounts = await m3sWallet.getAccounts();
        if (!accounts || accounts.length === 0) {
            throw new AdapterError("No accounts found in the M3S wallet instance.", { methodName: 'create', code: WalletErrorCode.InvalidInput });
        }
    ...
    public async switchChain(chainId: number): Promise<WalletClient> {
        ...
        if (preferredRpcs.length === 0) {
            throw new AdapterError(
                `No private RPCs configured for chain ${cidHex}. Please call wallet.updateAllChainRpcs before executing cross-chain operations.`,
                { methodName: 'switchChain', code: WalletErrorCode.InvalidInput }
            );
        }
        ...
        if (!targetNetworkConfig) {
           throw new AdapterError(`Could not get network config for chainId: ${chainId}`, { methodName: 'switchChain', code: WalletErrorCode.InvalidInput });
        }
        ...
        try {
            ...
            throw new AdapterError(`M3S wallet failed to switch to chain ${chainId}: ${m3sSwitchError.message}`, { methodName: 'switchChain', code: WalletErrorCode.InvalidInput });
        }
