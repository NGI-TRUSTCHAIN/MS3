...
export async function getNetworkConfig(
  ...
  const combinedUrls = [...new Set([...preferredRpcUrls, ...baseRpcUrls])]; // Preferred first, then base, deduplicated
  ...
  // 3. Test URLs Sequentially
  console.log(`[getNetworkConfig] Testing ${combinedUrls.length} RPC(s) for ${baseConfig.name} (Chain ID: ${baseConfig.chainId})...`);
  ...
  const networkConfig: NetworkConfig = {
    ...
    rpcUrls: orderedRpcUrls, // Return the full, ordered list
