...
export class EvmWalletAdapter implements IEVMWallet {
  ...
  */
  ...
  public async sendTransaction(tx: GenericTransactionData): Promise<string> {
    ...
    const signer = await this.getSigner();
    console.log('Sending tx from client: ', tx);

    // Prepare request and merge user-supplied fee/limit overrides (if any)
    const txRequest = await this.prepareTransactionRequest(tx);
    if (tx.options) {
      try {
        if (tx.options.gasLimit !== undefined) {
          txRequest.gasLimit = typeof tx.options.gasLimit === 'bigint' ? tx.options.gasLimit : BigInt(tx.options.gasLimit.toString ? tx.options.gasLimit.toString() : tx.options.gasLimit);
        }
        if (tx.options.maxFeePerGas !== undefined) {
          txRequest.maxFeePerGas = typeof tx.options.maxFeePerGas === 'bigint' ? tx.options.maxFeePerGas : BigInt(tx.options.maxFeePerGas.toString ? tx.options.maxFeePerGas.toString() : tx.options.maxFeePerGas);
        }
        if (tx.options.maxPriorityFeePerGas !== undefined) {
          txRequest.maxPriorityFeePerGas = typeof tx.options.maxPriorityFeePerGas === 'bigint' ? tx.options.maxPriorityFeePerGas : BigInt(tx.options.maxPriorityFeePerGas.toString ? tx.options.maxPriorityFeePerGas.toString() : tx.options.maxPriorityFeePerGas);
        }
        if (tx.options.gasPrice !== undefined) {
          txRequest.gasPrice = typeof tx.options.gasPrice === 'bigint' ? tx.options.gasPrice : BigInt(tx.options.gasPrice.toString ? tx.options.gasPrice.toString() : tx.options.gasPrice);
        }
        if (tx.options.type !== undefined) {
          // keep type as-is (number or string)
          (txRequest as any).type = tx.options.type;
        }
      } catch (err) {
        console.warn('[EvmWalletAdapter] Failed merging tx.options into request:', err);
      }
    }
    console.log('Sending tx from client (final txRequest): ', txRequest);
  ...
  public async prepareTransactionRequest(tx: GenericTransactionData): Promise<ethers.TransactionRequest> {
    ...
    if (txRequest.nonce === undefined) {
      ...
      try {
        ...
        console.warn('[EvmWalletAdapter] prepareTransactionRequest: failed merging options', err);
      }
    }
