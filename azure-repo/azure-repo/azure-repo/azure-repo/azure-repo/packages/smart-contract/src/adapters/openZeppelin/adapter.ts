...
export class OpenZeppelinAdapter implements IBaseContractHandler {
        ...
        try {
            ...
            if (adapterRpcUrl) {
                try {
                    ...
                    await this.defaultProvider.getNetwork();
    ...
    normalizeOzOptions(template: string, options: any): any {
        ...
        if (supportsUpgradeable) {
            ...
            if (opts.upgradeable && !allowed.includes(opts.upgradeable)) {
                // Remove invalid upgradeable option instead of setting to false
                delete opts.upgradeable;
            }
            if (opts.upgradeable === null) {
                delete opts.upgradeable;
            }
        }
