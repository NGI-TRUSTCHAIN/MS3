...
import { TEST_PRIVATE_KEY, RUN_INTEGRATION, INFURA_API_KEY } from '../../../config.js';
import { createWallet, IEVMWallet } from '@m3s/wallet';
import { Ms3Modules, NetworkHelper } from '@m3s/shared';
import * as node_path from 'path';
import * as fs_promises from 'fs/promises';
import { logger } from '../../../../../logger.js';
import { console } from 'inspector';
import {smartContractRegistry as registry} from '../../../src/index.js'
...
describe('OpenZeppelinAdapter Tests', () => {
  ...
  describe.skip('ERC20 Options Tests', () => {
    let contractHandler: IBaseContractHandler & { workDir: string }
  ...
  (RUN_INTEGRATION ? describe : describe.skip)('Full Integration Tests', () => {
    ...
    const setupTestEnvironmentInternal = async (preserveOutput: boolean, useTempWorkDir: boolean = false) => {
      ...
      const preferredRpcUrl = `https://ethereum-sepolia-rpc.publicnode.com`;
    ...
    describe('Standard ERC20 Deployment and Functionality', () => {
      ...
      it('should deploy ERC20 with multiple features and verify functionality', async () => {
        ...
        const buildOverridesFromEstimate = (est: any) => {
          const MIN_PRIORITY = 1_500_000_000n; // 1.5 gwei
          let gl: bigint | undefined;
          try {
            if (est?.gasLimit !== undefined) {
              gl = typeof est.gasLimit === 'bigint' ? est.gasLimit : BigInt(est.gasLimit.toString ? est.gasLimit.toString() : est.gasLimit);
              gl = (gl as bigint * 120n) / 100n;
            }
          } catch { gl = undefined; }

          const maxPriority = est?.maxPriorityFeePerGas ? BigInt(est.maxPriorityFeePerGas.toString ? est.maxPriorityFeePerGas.toString() : est.maxPriorityFeePerGas) : undefined;
          const maxFee = est?.maxFeePerGas ? BigInt(est.maxFeePerGas.toString ? est.maxFeePerGas.toString() : est.maxFeePerGas) : undefined;

          const priority = (maxPriority && maxPriority > MIN_PRIORITY) ? maxPriority : MIN_PRIORITY;
          const fee = maxFee && maxFee > (priority) ? maxFee : (priority * 3n); // conservative fallback
        ...
        // Estimate and apply overrides for deployment (no `to`)
        const deployEstimate = await walletAdapter.estimateGas({
          to: undefined,
          data: deploymentData.data,
          value: deploymentData.value || '0'
        });

        const deployOverrides = buildOverridesFromEstimate(deployEstimate);

        try {
          deployOverrides.gasLimit = deployOverrides.gasLimit ? BigInt(deployOverrides.gasLimit) : 1800000n;
        } catch {
          deployOverrides.gasLimit = 1800000n;
        }

        deployOverrides.maxPriorityFeePerGas = 3_000_000_000n; // 3 gwei
        deployOverrides.maxFeePerGas = 15_000_000_000n; // 15 gwei
        console.log('DEBUG: forcing deployOverrides =>', deployOverrides);

        const deployTxHash = await walletAdapter.sendTransaction({
          data: deploymentData.data,
          value: deploymentData.value || '0',
          options: deployOverrides
        });

        const deployReceipt = await waitForReceipt(deployTxHash);
        logger.info('deployReceipt, ', deployReceipt);
        ...
        const deployerBalanceRaw = await walletAdapter.callContract({
          contractAddress,
          abi: compiled.artifacts.abi,
          method: 'balanceOf',
          args: [deployerAddress]
        ...
        const deployerBalance = deployerBalanceRaw[0] as bigint;
        const desiredBurn = ethers.parseUnits('100', 18);
        const burnAmount = deployerBalance > 0n ? (desiredBurn <= deployerBalance ? desiredBurn : deployerBalance) : 1n;
    ...
    describe.skip('Proxy Artifact Preservation Tests', () => {
    ...
    describe.skip('General Proxy Functionality Tests', () => {
      beforeEach(async () => {
