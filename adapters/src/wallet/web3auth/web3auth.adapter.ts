...
import { AdapterArguments, IEVMWallet, AdapterError, WalletErrorCode, WalletEvent, AssetBalance, NetworkConfig, GenericTransactionData, EIP712TypedData, EstimatedFeeData } from "@m3s/wallet";
...
interface args extends AdapterArguments<IWeb3AuthWalletOptionsV1> { 
  name: string,
  version: string,
  options: IWeb3AuthWalletOptionsV1
}
  ...
  async requestAccounts(): Promise<string[]> {
    ...
    try {
      if (this.web3auth.connected && this.web3auth.provider) {
        const accounts = await this.getAccounts(); // Should use existing provider
  ...
  private async prepareTransactionRequest(tx: GenericTransactionData): Promise<ethers.TransactionRequest> {
    ...
    if (txRequest.gasLimit === undefined && tx.options?.gasLimit === undefined) {
      if (!txRequest.data || txRequest.data === "0x") {
        ...
        // console.debug("[Web3AuthWalletAdapter] Simple ETH transfer, setting gasLimit to 21000 for tx prep.");
        txRequest.gasLimit = BigInt(21000);
      } else {
        // For contract interactions, attempt estimation ONLY if not already populated by options
        // console.debug("[Web3AuthWalletAdapter] Contract interaction, attempting to estimate gas for tx prep:", txRequest);
        try {
          const estimatedGas = await provider.estimateGas(txRequest); // provider from this.getProvider()
          txRequest.gasLimit = estimatedGas;
          // console.debug("[Web3AuthWalletAdapter] Estimated gas for tx prep:", estimatedGas.toString());
        } catch (e: any) {
          console.warn(`[Web3AuthWalletAdapter] Could not estimate gas for tx prep (contract interaction). Error: ${e.message}. Tx may fail if gasLimit not populated by signer.`, txRequest);
          // Let ethers.js populateOrThrow if estimation fails here.
          // Or, rethrow if this estimation is critical before signTransaction.
          // For signTransaction, it's often better to let the signer handle population if estimation here fails.
        }
      }
    }
